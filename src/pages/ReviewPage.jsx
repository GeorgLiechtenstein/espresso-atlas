...
// Detect-and-translate via the Netlify proxy. Returns
// { sourceLang: 'de'|'en', translation: string } or null on failure.
...
export default function ReviewPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('venues').select('*').eq('id', preVenueId).single()
      .then(({ data }) => {
        if (!data) return;
        setName(data.name || '');
        // Show whichever language matches the active app lang in the
        // editable form fields. The save flow re-resolves both columns
        // from coordinates via Nominatim, so the form value is just a
        // hint and a fallback if Nominatim is unreachable at save time.
        setCity(venueCity(data, lang));
        setCountry(venueCountry(data, lang));
  ...
  function applyLocSuggestion(s) {
    ...
    // The suggestion came back in the current app language; that fills
    // the visible form fields. The save flow re-resolves both languages
    // from coordinates, so it doesn't matter which variant lands here.
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Bilingual city / country resolution. Two Nominatim reverse
      // calls (DE + EN) at the resolved coordinates — Nominatim returns
      // the official localised name in each language, so we get the
      // canonical pair without guessing or running text through Claude.
      // Falls back to the typed values + normalizeCountry if Nominatim
      // is unreachable; the user's input always wins over an empty
      // result.
      const cn = normalizeCountry(country) || { de: country.trim(), en: country.trim() };
      let cityFinal      = city.trim();
      let cityEnFinal    = city.trim();
      let countryFinal   = cn.de;
      let countryEnFinal = cn.en;
      if (resolvedLat && resolvedLng) {
        try {
          const [resDe, resEn] = await Promise.all([
            fetch(`https://nominatim.openstreetmap.org/reverse?lat=${resolvedLat}&lon=${resolvedLng}&format=json&addressdetails=1`, { headers: { 'Accept-Language': 'de' } }),
            fetch(`https://nominatim.openstreetmap.org/reverse?lat=${resolvedLat}&lon=${resolvedLng}&format=json&addressdetails=1`, { headers: { 'Accept-Language': 'en' } }),
          ]);
          const [dataDe, dataEn] = await Promise.all([resDe.json(), resEn.json()]);
          const aDe = dataDe.address || {};
          const aEn = dataEn.address || {};
          const cd = aDe.city || aDe.town || aDe.village || aDe.municipality || '';
          const ce = aEn.city || aEn.town || aEn.village || aEn.municipality || '';
          if (cd) cityFinal      = cd;
          if (ce) cityEnFinal    = ce;
          if (aDe.country) countryFinal   = aDe.country;
          if (aEn.country) countryEnFinal = aEn.country;
        } catch { /* silent — keep the typed-value fallbacks above */ }
      }

      // Bilingual comment. Single round-trip — Claude detects the source
      // language from the text and returns the translation in the other.
      // The original text always lands in comment_<sourceLang>; the
      // returned translation always lands in the opposite column. No
      // client-side heuristic, no app-lang fallback, no risk of getting
      // the columns swapped.
      const trimmedComment = comment.trim() || null;
      let comment_de = null;
      let comment_en = null;
      if (trimmedComment) {
        const result = await detectAndTranslate(trimmedComment);
        if (result && (result.sourceLang === 'de' || result.sourceLang === 'en') && result.translation) {
          if (result.sourceLang === 'de') {
            comment_de = trimmedComment;
            comment_en = result.translation;
          } else {
            comment_en = trimmedComment;
            comment_de = result.translation;
          }
        } else {
          // Translation service unavailable — store the original in
          // whichever column matches the active app language. Other
          // column stays null until next save.
          if (lang === 'de') comment_de = trimmedComment;
          else               comment_en = trimmedComment;
        }
      }
